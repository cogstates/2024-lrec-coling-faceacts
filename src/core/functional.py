...
def cache(path: str) -> Callable[..., Any]:
    ...
    def decorator(fn: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def grapper(*args: Any, **kwargs: Any) -> Any:
        ...
        def clear() -> list[str]:
            shelfs = glob.glob(f"{path}.shelf.*")
            list(map(os.remove, shelfs))
            return shelfs

        setattr(wrapper, "clear", clear)
        setattr(grapper, "clear", clear)
        return grapper if inspect.isgeneratorfunction(fn) else wrapper
    return decorator
# fmt: on
